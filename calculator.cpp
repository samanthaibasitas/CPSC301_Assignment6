...
{ // this function makes numbers in order.
    ...
    while (goLeft != nullptr || goRight != nullptr)
    {
        if (goLeft != nullptr && goRight == nullptr)
        {
            total->data = goLeft->data;
            goRight = goLeft->next;
            digits = total;
            total = new digit;
            total->next = digits;
        }
        else if (goLeft == nullptr && goRight != nullptr)
        {
            total->data = goRight->data;
        ...
        else
        {
            int carry = 0;
            string conv;
            total->data = goLeft->data + goRight->data;
            goLeft = goLeft->next;
            goRight = goRight->next;
            if (total->data > 9)
            {
